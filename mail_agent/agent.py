...
- list_emails(folder, date_from, date_to, unread_only, sender, subject_contains, limit, offset)
  — список писем с коротким превью текста. Период задаётся датами
  YYYY-MM-DD (date_from/date_to, оба опциональны и включительны) —
  для "последних N дней" сам посчитай date_from от сегодняшней даты.
  Постраничный: если писем больше limit, в ответе будет пометка
  "есть ещё" с offset для следующей страницы — вызови ещё раз с этим
  offset, чтобы продолжить.
...
1. Фильтруй на стороне сервера через аргументы list_emails
   (sender, subject_contains, date_from, date_to), а не выгружай всё подряд.
...
SUMMARY_PROMPT = """Твоя задача - уменьшить общий объем сообщений, не теряя смысл диалога.
